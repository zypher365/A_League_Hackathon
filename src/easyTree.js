        ...
        this.each(function () {
            ...
            $.each($(easyTree).find('ul > li'), function() {
                ...
                if($(this).is('li:has(ul)')) {
                    ...
                    text = $(this).text();
                    $(this).html('<span><span class="glyphicon"></span><a href="javascript: void(0);"></a> </span>');
                    $(this).find(' > span > span').addClass('glyphicon-flag');
                    $(this).find(' > span > a').text(text);
                }
            });
            ...
            if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        var item = $('<li><span><span class="glyphicon glyphicon-flag"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                        ...
                        if (selected.length <= 0) {
                            ...
                            if ($(selected).hasClass('parent_li')) {
                                ...
                                $(selected).addClass('parent_li').find(' > span > span').addClass('glyphicon-minus').removeClass('glyphicon-flag');
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content .confirm').on('click', function () {
                            ...
                            if($(selected).parent('ul').find(' > li').length <= 1) {
                                $(selected).parents('li').removeClass('parent_li').find(' > span > span').removeClass('glyphicon-minus').addClass('glyphicon-flag');
            ...
            if (options.selectable) {
                ...
                $(easyTree).find('li > span > a').click(function (e) {
                    ...
                    if (li.hasClass('li_selected')) {
                        ...
                        var elemID =  $(li).attr('id');

                        switch (elemID){
                            case ('mainline'):
                                $("#tabLoad").load("powerlines/mainline/index.html");
                                $("#simplifiedView").load("powerlines/mainline/simplified.html");
                                break;

                            case ('elec_room_2'):
                                $("#tabLoad").load("powerlines/elec_room_2/index.html");
                                $("#simplifiedView").load("powerlines/elec_room_2/simplified.html");
                                break;
                            case ('e1_mixure'):
                                $("#tabLoad").load("powerlines/elec_room_1-mixure/index.html");
                                $("#simplifiedView").load("powerlines/elec_room_1-mixure/simplified.html");
                                break;
                            case ('e1_utility'):
                                $("#tabLoad").load("powerlines/elec_room_1-utility/index.html");
                                $("#simplifiedView").load("powerlines/elec_room_1-utility/simplified.html");
                                break;
                            case ('e1_grinder'):
                                $("#tabLoad").load("powerlines/elec_room_1-grinder/index.html");
                                $("#simplifiedView").load("powerlines/elec_room_1-grinder/simplified.html");
                                break;
                            case ('elec_room_1'):
                                $("#tabLoad").load("powerlines/elec_room_1/index.html");
                                $("#simplifiedView").load("powerlines/elec_room_1/simplified.html");
                                break;
